...
const Chat_component = () => {
  ...
  useEffect(() => {
    const token = getCompanyAccessToken();
      ...
      setCompanyWhatsapp(decodedToken.phone)
  ...
  const fetchEmployees = async () => {
    try {
      ...
      const employeesResponse = await axios.get(
        `${process.env.NEXT_PUBLIC_BASE_URL}/company/get-all-employees/${company_id}`,
        {
          headers: {
  ...
  const fetchCustomers = async () => {
    try {
      ...
      const customersResponse = await axios.get(
        `${process.env.NEXT_PUBLIC_BASE_URL}/company/get-all-customers/${company_id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleCheckboxChange = (contactId, e) => {
    ...
    if (existingIndex !== -1) {
      ...
      const chatsToAdd = allChats.filter(
        (chat) => chat.customer_id === contactId
      );
  ...
  const handleMergeChats = () => {
    ...
    mergeChats(loggedInUserId, chatIdsToMerge, mergeName); // Assuming loggedInUserId is accessible here
    closeMergeModal();
  };
  ...
  //   console.log(option); // Implement the move to auto/manual functionality here
  //   setShowOptions(false);
  // };
