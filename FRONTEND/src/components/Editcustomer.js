  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        const customerId = searchParams.get('ct_id');
        
        const { confirmPassword, ...requestData } = values;
        // Replace this with your actual API call to update employee details
        const token = getAdminAccessToken();
        const response = await axios.put(
          `${process.env.NEXT_PUBLIC_BASE_URL}/admin/edit-customer-by-admin/${customerId}`,
          {
    ...
    const fetchCustomerDetails = async () => {
      try {
        ...
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_BASE_URL}/admin/get-single-customer-by-admin/${companyId}/${customerId}`,{
