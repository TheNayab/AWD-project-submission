  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        const response = await axios.put(`${process.env.NEXT_PUBLIC_BASE_URL}/admin/edit-company/${companyId}`, requestData, {
          headers: {
  ...
  useEffect(() => {
    const fetchCompanyDetails = async () => {
      try {
        const companyId = searchParams.get('id');
        const token = getAdminAccessToken();
        const response = await axios.get(`${process.env.NEXT_PUBLIC_BASE_URL}/admin/get-single-company/${companyId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
      ...
      const { name, email, phone, user_name,password } = data.company;
      ...
      formik.setValues({
        ...
        user_name: user_name || "",
        password: password,  
        confirmPassword: "",  // don't want to update the password here
      });
    } catch (error) {
